 ...
 * Footer: Clean, warm footer with links and NZ branding
 */
  ...
  return (
    <footer className="bg-espresso text-oat/60 pt-16 pb-8">
      <div className="container">
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-10 md:gap-12 mb-12">
          {/* Brand */}
          <div className="lg:col-span-2">
            ...
            <p className="text-sm text-oat/40 leading-relaxed max-w-xs mb-5" style={{ fontFamily: "var(--font-body)" }}>
              AI-powered loyalty and review platform for New Zealand and Australia's independent cafés and restaurants. More reviews, more regulars, zero effort.
            </p>
            <div className="flex items-center gap-2 text-xs text-oat/30" style={{ fontFamily: "var(--font-body)" }}>
              <MapPin size={14} />
              <span>Made with aroha in Aotearoa, New Zealand</span>
            </div>
          </div>
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4
                className="text-sm font-semibold text-oat/80 uppercase tracking-wider mb-4"
              ...
              <ul className="space-y-2.5">
                {links.map((link) => (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      className="text-sm text-oat/40 hover:text-kowhai-amber transition-colors duration-300"
                      style={{ fontFamily: "var(--font-body)" }}
                    >
                      {link.label}
                    </a>
